...
# -*- coding: utf-8 -*-
"""
简化的GPT模型训练脚本 - 使用原始GPT-SoVITS训练方式
"""
...
logger = logging.getLogger('train_gpt_simple')

def main():
    parser = argparse.ArgumentParser(description='简化的GPT模型训练')
    ...
    parser.add_argument('--max-epochs', type=int, default=10,
                        help='最大训练轮数 (默认: 10)')
    parser.add_argument('--device', type=str, default='cuda' if 'torch' in sys.modules and sys.modules['torch'].cuda.is_available() else 'cpu',
                        help='设备 (默认: auto)')
    ...
    name2semantic_path = data_dir / '6-name2semantic.tsv'
    
    if not name2text_path.exists():
        logger.error(f"文本文件不存在: {name2text_path}")
        sys.exit(1)
    
    if not name2semantic_path.exists():
        logger.error(f"语义文件不存在: {name2semantic_path}")
        sys.exit(1)
    ...
    logger.info(f"开始训练{args.language}语言的GPT模型...")
    ...
    gpt_train_script = Path("GPT_SoVITS/s1_train.py")
    if gpt_train_script.exists():
        logger.info("使用GPT-SoVITS原始训练脚本")
        try:
            cmd = [
                sys.executable, str(gpt_train_script),
                "--text_path", str(name2text_path),
                "--semantic_path", str(name2semantic_path),
